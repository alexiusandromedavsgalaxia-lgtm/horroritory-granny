...
class HorrorView extends View {
    ...
    float enemyMult(){return new float[]{.7f,1f,1.15f,1.3f,1.5f,2f,2.5f,3f}[selected];}
    ...
    @Override protected void onDraw(Canvas c){super.onDraw(c); if(menu)drawMenu(c); else {update(.016f); drawWorld(c); if(dead||won)drawEnding(c);} postInvalidateDelayed(16); }
    ...
    void drawWorld(Canvas c){bg(c);p.setStyle(Paint.Style.FILL);p.setColor(Color.rgb(54,50,46));for(float[]w:walls)c.drawRect(w[0],w[1],w[2],w[3],p);
        ...
        label(c,60,30,"MODO: "+modes[selected]);label(c,60,52,"OBJETOS: "+keys+"/3   ·   toca E para recoger");
        if(selected==7){p.setShader(new RadialGradient(px,py,160,new int[]{0x00000000,0xCC000000,0xF0000000},new float[]{0,.55f,1},Shader.TileMode.CLAMP));c.drawRect(0,0,getWidth(),getHeight(),p);p.setShader(null);label(c,60,getHeight()-24,"AE: VISIÓN 10% · PERSEGUIDORA: VISIÓN COMPLETA · CRUJIDOS 80%+");}
        drawTouch(c);
    }
    ...
    void drawMenu(Canvas c){c.drawColor(Color.rgb(5,5,7));p.setTextAlign(Paint.Align.CENTER);p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(54);p.setColor(Color.WHITE);c.drawText("HORRORITORY",getWidth()/2f,150,p);p.setTextSize(18);p.setTypeface(Typeface.DEFAULT);c.drawText("escapa de la casa",getWidth()/2f,185,p);p.setTextSize(24);c.drawText("MODO: "+modes[selected],getWidth()/2f,270,p);p.setTextSize(16);c.drawText("toca la zona izquierda/derecha para cambiar",getWidth()/2f,305,p);p.setTextSize(26);p.setColor(Color.LTGRAY);c.drawText("EMPEZAR",getWidth()/2f,410,p);p.setTextSize(13);p.setColor(Color.GRAY);c.drawText("Android · paisaje · offline",getWidth()/2f,450,p);p.setTextAlign(Paint.Align.LEFT); }
    void drawEnding(Canvas c){p.setColor(0xDD000000);c.drawRect(0,0,getWidth(),getHeight(),p);p.setTextAlign(Paint.Align.CENTER);p.setColor(Color.WHITE);p.setTextSize(36);c.drawText(won?"HAS ESCAPADO":"TE HAN ENCONTRADO",getWidth()/2f,220,p);p.setTextSize(17);if(won){String[] places={"una lavandería abandonada","un cine sin público","un vagón de tren vacío","un invernadero en un tejado","un supermercado que no aparece en ningún mapa"};c.drawText("FINAL ALTERNATIVO: "+places[rng.nextInt(places.length)],getWidth()/2f,270,p);}else c.drawText("la casa vuelve a quedarse en silencio...",getWidth()/2f,270,p);c.drawText("toca para volver a jugar",getWidth()/2f,350,p);p.setTextAlign(Paint.Align.LEFT); }
    void label(Canvas c,float x,float y,String s){p.setTextSize(13);p.setColor(Color.WHITE);c.drawText(s,x,y,p);}
    @Override public boolean onTouchEvent(MotionEvent e){float x=e.getX(),y=e.getY();int a=e.getActionMasked();if(a==MotionEvent.ACTION_DOWN){if(menu){if(y>230&&y<335){if(x<getWidth()/2f)selected=(selected+modes.length-1)%modes.length;else selected=(selected+1)%modes.length;invalidate();return true;}if(y>350&&y<480){startGame();return true;}}else if(dead||won){startGame();return true;}else{moving=true;updateJoy(x,y);if(x>getWidth()-170&&y>getHeight()-180)collect();}}else if(a==MotionEvent.ACTION_MOVE&&!menu&&!dead&&!won){updateJoy(x,y);}else if(a==MotionEvent.ACTION_UP){moving=false;joyX=joyY=0;}return true;}
    void updateJoy(float x,float y){if(y>getHeight()-190&&x<240){float dx=x-100,dy=y-(getHeight()-100);joyX=Math.max(-1,Math.min(1,dx/65));joyY=Math.max(-1,Math.min(1,dy/65));}}
    ...
    static class Enemy{float x,y;Enemy(float x,float y){this.x=x;this.y=y;}void update(float dt,HorrorView g){float dx=g.px-x,dy=g.py-y,d=(float)Math.hypot(dx,dy);if(d<520&&d>1){x+=dx/d*105*g.enemyMult()*dt;y+=dy/d*105*g.enemyMult()*dt;}else{x+=(float)Math.sin(System.nanoTime()/1e9+x)*12*dt;y+=(float)Math.cos(System.nanoTime()/1e9+y)*12*dt;}}}
